...
class insert_w
{
    ...
    public function insert_w()
    {
        // correlativo
        $row = $this->db
            ->query("SELECT (MAX(wguidenumber) + 1) AS ngrmasuno FROM tb_warranty")
            ->fetch_assoc();

        $ngr = $row['ngrmasuno'] ?? 1;
        $entryCode = $this->generarCodigo(10);

        $sql = "
            INSERT INTO tb_warranty (
                ccodcli, ucoduser,
                guidesource, guidenumbersource,
                wendpoint, wguidenumber,
                wvoucher, wpriceproduct,
                codigo, descripcion,
                categoria, subcategoria,
                marca, waccessories, serialnumber,
                wprpc, wequipmentstatus, wdiagnostic,
                wproblemsdetected, wconcludingremarks,
                wentrydate, westimateddate,
                wguidetype, wproductlocation,
                wentrycode, wstage
            )
            VALUES (
                ?, ?, 
        ...
        // indicador
        $sqlIndicator = "
            INSERT INTO tb_warranty_indicator (wguidenumber, wentrydate)
            VALUES (?, NOW())
        ";

        $stmt = $this->db->prepare($sqlIndicator);
        $stmt->bind_param("s", $ngr);
        $stmt->execute();
