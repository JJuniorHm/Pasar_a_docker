...
$class_insert = new class_insertClient();
    ...
    function validateOnlyLetters($input) {
        return !empty(trim($input)) && preg_match('/^[a-zA-Z\sáéíóúÁÉÍÓÚñÑ]+$/', $input);
    }
    ...
    function validateinputs(){
        if($_POST['typeregister'] === 'savecompanie'){
            if(!validateOnlyNumbers(str_replace(' ', '', $_POST['ccodcli']))){
        ...
        if( strlen(str_replace(' ', '', $_POST['ccodcli'])) < 8){
        ...
        if(!validateOnlyNumbers(str_replace(' ', '', $_POST['ctelefono1']))){
        ...
        if(!empty($_POST['ctelefono2'])){
            if(!validateOnlyNumbers(str_replace(' ', '', $_POST['ctelefono2']))){
    ...
    if($_POST['typeregister'] === 'saveclient'){
        ...
        $class_insert->SetCCodCli(str_replace(' ', '', $_POST['ccodcli']));
        ...
        $class_insert->SetCTelefono1(str_replace(' ', '', $_POST['ctelefono1']));
        $class_insert->SetCTelefono2(str_replace(' ', '', $_POST['ctelefono2']));
